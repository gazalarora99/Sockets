...
if(portno < 5000 || portno > 64000){ 
	printf("The port number does not fall within correct range \n"); 
	exit(1);
}
...
if(bind(sockfd,(struct sockaddr *)&serverAddressInfo, sizeof(serverAddressInfo))<0){
  printf("error binding, port maybe in use, make sure no other server is using the same port, try different port number\n");
  exit(1);
}
  ...
  listen(sockfd,MAX_CUE); 
				...
				*/
				if(strcmp(res, "close") == 0){
				  ...
				  printf("Incorrect message 1 from client, sending error %s, closing connection\n", res);
				  free(res);
				  close(newsockfd);
                                  continue;
				}
				...
				res = check(newsockfd, buffer, 18, 3); //receiving msg 3 JA Francisco, who?
				  ...
				  continue;
                                  ...
                                  write(newsockfd, res, 9); //need to send an error to client
				  printf("Incorrect message 3 from client, sending error %s, closing connection\n", res);
                                  free(res);
				  close(newsockfd);
                                  continue;
				}
				...
				if(strcmp(res, "close") == 0){
				  ...
				  continue;
                                  ...
                                  write(newsockfd, res, 9); //need to send an error to client
				  printf("Incorrect message 5 from client, sending error %s, closing connection\n", res);
                                  free(res);
				  close(newsockfd);
                                  continue;
                                }
